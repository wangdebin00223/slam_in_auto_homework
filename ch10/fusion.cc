...
/***
这段代码的主要功能是通过RTK数据进行定位初始化，具体步骤包括：
检查初始化是否失败，如果失败且位置变化不大，则跳过搜索。
加载地图数据。
在360度范围内按固定步长生成搜索姿态。
并行计算每个姿态的匹配得分。
选择得分最高的匹配结果。
如果得分超过阈值，则更新滤波器状态并返回成功；否则，标记初始化失败并返回失败。
通过这种方式，代码实现了基于RTK的定位初始化，为后续的融合定位提供了准确的初始状态。
***/
bool Fusion::SearchRTK() {
    ...
    //std::for_each：遍历search_poses向量。
    // std::execution::par_unseq：启用并行执行，加速搜索过程。
    // AlignForGrid(gr)：对每个搜索姿态进行对齐操作，计算匹配得分。
    ...
    if (max_ele->score_ > rtk_search_min_score_) {
        ...
        //std::max_element：从search_poses中找到得分最高的匹配结果。
        // g1.score_ < g2.score_：比较两个匹配结果的得分。
        // LOG(INFO)：输出最高得分及其对应的姿态。
        ...
        // eskf_.GetNominalState()：获取滤波器的当前状态。
        // state.R_ = max_ele->result_pose_.so3();：将最优匹配结果的旋转部分赋值给状态。
        // state.p_ = max_ele->result_pose_.translation();：将最优匹配结果的平移部分赋值给状态。
        // state.v_.setZero();：将速度设置为零。
        // eskf_.SetX(state, eskf_.GetGravity());：更新滤波器的状态
...
void Fusion::LoadMap(const SE3& pose) {
    ...
    /***
    iter->first 是当前区块的网格索引。
    key 是当前机器人所在网格的索引。    
    (iter->first - key).cast<float>().norm() 计算当前区块与机器人所在网格的距离。
    如果距离大于 3.0，则卸载该区块。 
    ***/
