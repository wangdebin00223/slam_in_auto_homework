...
void LoamLikeOdom::ProcessPointCloud(FullCloudPtr cloud) {
    ...
    if (IsKeyframe(pose)) {
        ...
        // 将当前关键帧的边缘特征点和平面特征点加入 edges_ 和 surfs_ 中。
        // 如果 edges_ 或 surfs_ 的大小超过了局部地图中关键帧的最大数量 options_.num_kfs_in_local_map_，则移除最早的关键帧。
        ...
        //对局部地图中的平面特征点和边缘特征点进行降采样（体素滤波），以减少点云密度。
...
SE3 LoamLikeOdom::AlignWithLocalMap(CloudPtr edge, CloudPtr surf) {
    ...
    for (int iter = 0; iter < options_.max_iteration_; ++iter) {
        ...
        //点面残差是点到平面的垂直距离，是一个标量值（只有大小，没有方向）。
        ...
        //点线残差是指一个点到目标直线的垂直距离向量，它描述了点在垂直于直线方向上的偏差，因此是 3 维的。
