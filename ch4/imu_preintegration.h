...
class IMUPreintegration {
   public:
    ...
    SO3 GetDeltaRotation(const Vec3d &bg);  //得到修正之后的旋转
    Vec3d GetDeltaVelocity(const Vec3d &bg, const Vec3d &ba);   //得到修正之后的速度
    Vec3d GetDeltaPosition(const Vec3d &bg, const Vec3d &ba);   //得到修正之后的位移
