...
void LikelihoodField::BuildModel() {
    ...
    //构建一个 似然场模型（Likelihood Field Model）
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        ...
        for (size_t i = 0; i < source_->ranges.size(); ++i) {
            ...
            if (pf[0] >= image_boarder && pf[0] < field_.cols - image_boarder && pf[1] >= image_boarder &&
                ...
                // 图像梯度
                // dx 和 dy：分别表示似然场在 x 和 y 方向的梯度（通过中心差分法计算）
                // 直接通过整数坐标访问相邻像素值
                // 这里的pf[0]和pf[1]是整数坐标，直接取左右（或上下）两个相邻像素值的差值。
                // 这种方法假设坐标是精确的整数，未考虑非整数坐标的情况，属于普通差值
                ...
                // 通过双线性插值函数GetPixelValue_bilinear获取像素值
                // 即使pf[0] + 1或pf[1]是浮点数，双线性插值会根据周围四个像素点进行加权计算，得到更平滑的估计值。
                // 这适用于坐标可能为浮点数的情况（如图像旋转、缩放后），提升了梯度计算的精度
                // 图像梯度（双线性插值 bilinear interpolation）
                float dx = 0.5 * (math::GetPixelValue<float>(field_, pf[0] + 1, pf[1]) - math::GetPixelValue<float>(field_, pf[0] - 1, pf[1]));
                float dy = 0.5 * (math::GetPixelValue<float>(field_, pf[0], pf[1] + 1) - math::GetPixelValue<float>(field_, pf[0], pf[1] - 1));
...
cv::Mat LikelihoodField::GetFieldImage() {
    ...
    for (int x = 0; x < field_.cols; ++x) {
        for (int y = 0; y < field_.rows; ++y) {
            ...
            //除以 30.0：这是一个归一化因子，可能是为了将 field_ 的值缩放到 [0, 255] 范围内
