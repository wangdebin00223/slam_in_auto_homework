...
class EdgeSEP2P : public g2o::BaseUnaryEdge<2,Vec2d,VertexSE2>{
    public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    EdgeSEP2P(double range, double angle, Vec2d pw, double theta):range_(range),angle_(angle),theta_(theta),qw(qw){
    }

    void computeError() override {
        auto *pose = dynamic_cast<VertexSE2*>(_vertices[0]);
        _error = pose->estimate( )*Vec2d(range_ * std::cos(angle_ + theta_),range_ * std::sin(angle_ + theta_)) - qw;
    }

    void linearizeOplus() override {
        _jacobianOplusXi << 1, 0, 0, 1, 
                            -range_ * std::sin(angle_ + theta_), range_ * std::cos(angle_ + theta_);
    }
